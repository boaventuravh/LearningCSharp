...
public class Rgb //essa classe como um todo é o item i da lista
{
	...
	//os atributos e o método abaixo são refentes ao item xvi
	public int[] preta = new int[3] {0,0,0};
	public int[] branca = new int[3] { 255, 255, 255 };
	public int[] vermelha = new int[3] { 255, 0, 0 };
	public int[] verde = new int[3] { 0, 255, 0 };
	public int[] azul = new int[3] { 0, 0, 255 };

	public void SelecionarCor(Rgb cor, string escolha)
    {
		if(escolha == "preta")
        {
			cor.red = preta[0];
		...
		else if (escolha == "branca")
		{
			cor.red = branca[0];
			cor.green = branca[1];
			cor.blue = branca[2];
		}
		else if (escolha == "vermelha")
		{
			cor.red = vermelha[0];
			cor.green = vermelha[1];
			cor.blue = vermelha[2];
		}
		else if (escolha == "verde")
		{
			cor.red = verde[0];
			cor.green = verde[1];
			cor.blue = verde[2];
		}
		else if (escolha == "azul")
		{
			cor.red = azul[0];
			cor.green = azul[1];
			cor.blue = azul[2];
